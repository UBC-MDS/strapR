...
---
`strapvizr` is a package for performing bootstrapping of a sample to produce plots and statistics for use in final reports and documents. This notebook shows how you can utilize the strapvizpy package within a project.
...
stat_list_1 <- strapvizr::calculate_boot_stats(ex_data, 100, level = 0.95, seed = 123)
stat_list_1
```
...
stat_list_2 <- strapvizr::calculate_boot_stats(ex_data, 50, level = 0.90, seed = 123, 
                                estimator = "var", pass_dist = TRUE)
stat_list_2
```
...
plot_2 <- strapvizr::plot_ci(ex_data, rep=1000, bin_size=50, level=0.99, title=title,
